...
async def download_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if is_youtube:
            # YouTube - используем Android клиент
            ydl_opts.update({
                'extractor_args': {
                    'youtube': {
            ...
            # TikTok - используем мобильный API
            ydl_opts.update({
                'http_headers': {
        ...
        if 'sign in' in error_msg or 'bot' in error_msg:
            await status_message.edit_text(
                f"❌ YouTube требует авторизацию.\n\n"
                f"К сожалению, YouTube блокирует скачивание с серверов.\n"
                f"Попробуйте видео с TikTok или Instagram."
            )
        elif 'not available' in error_msg or 'status code 0' in error_msg:
            await status_message.edit_text(
                f"❌ Видео недоступно.\n\n"
                f"Возможные причины:\n"
                f"• Видео удалено или приватное\n"
                f"• Платформа блокирует скачивание\n"
                f"• Региональные ограничения\n\n"
                f"Попробуйте другую ссылку."
            )
        elif 'private' in error_msg or 'login' in error_msg:
            await status_message.edit_text(
                f"❌ Видео приватное или требует авторизации.\n\n"
                f"Я могу скачивать только публичные видео."
            )
        else:
            await status_message.edit_text(
                f"❌ Не удалось скачать видео.\n\n"
                f"Попробуйте другую ссылку или другую платформу."
            )
...
async def main():
    ...
    if sys.platform != 'win32':
        # Для Linux/Unix используем add_signal_handler
        loop = asyncio.get_running_loop()
        for sig in (signal.SIGTERM, signal.SIGINT):
            loop.add_signal_handler(sig, lambda: stop_event.set())
    else:
        # Для Windows используем обычную обработку через try/except
        pass
